...
const textAndTranslationToToken:  MapLike<{ text: string, token: SyntaxKind }> = {
    ...
    bryt: { text: "break", token: SyntaxKind.BreakKeyword },
    tilfelle: { text: "case", token: SyntaxKind.CaseKeyword },
    ...
    velg: { text: "switch", token: SyntaxKind.SwitchKeyword },
    ...
    immens: { text: "while", token: SyntaxKind.WhileKeyword },
...
export const textToKeywordObj: MapLike<KeywordSyntaxKind> = {
    ...
    bryt: SyntaxKind.BreakKeyword,
    tilfelle: SyntaxKind.CaseKeyword,
    ...
    immens: SyntaxKind.WhileKeyword,
...
export function createScanner(
    ...
    var scanner: Scanner = {
        ...
        hasPrecedingJSDocComment: () =>
            (tokenFlags & TokenFlags.PrecedingJSDocComment) !== 0,
        isIdentifier: () =>
        ...
        getNumericLiteralFlags: () =>
            tokenFlags & TokenFlags.NumericLiteralFlags,
        getTokenFlags: () => tokenFlags,
        reScanGreaterToken,
    ...
    if (Debug.isDebugging) {
        Object.defineProperty(scanner, "__debugShowCurrentPositionInText", {
            get: () => {
                ...
                return (
                    text.slice(0, scanner.getTokenFullStart()) +
                    "║" +
                    text.slice(scanner.getTokenFullStart())
                );
            },
        });
    }
    ...
    function scanNumber(): SyntaxKind {
        ...
        if (text.charCodeAt(pos) === CharacterCodes._0) {
            ...
            if (text.charCodeAt(pos) === CharacterCodes._) {
                tokenFlags |=
                    TokenFlags.ContainsSeparator |
                    TokenFlags.ContainsInvalidSeparator;
                error(
                    Diagnostics.Numeric_separators_are_not_allowed_here,
                    pos,
                    1
            ...
            // Separators are not allowed in the below cases
                ...
                const literal =
                    (withMinus ? "-" : "") + "0o" + (+tokenValue).toString(8);
                if (withMinus) start--;
                error(
        ...
        while (true) {
            ...
            if (ch === CharacterCodes.backslash && !jsxAttributeString) {
                ...
                result += scanEscapeSequence(
                    /*shouldEmitInvalidEscapeError*/ true
                );
                start = pos;
                continue;
            }
            ...
            switch (ch) {
                ...
                case CharacterCodes.exclamation:
                    if (text.charCodeAt(pos + 1) === CharacterCodes.equals) {
                        if (
                            text.charCodeAt(pos + 2) === CharacterCodes.equals
                        ) {
                            return (
                                (pos += 3),
                                (token =
                ...
                case CharacterCodes.percent:
                    if (text.charCodeAt(pos + 1) === CharacterCodes.equals) {
                        return (
                            (pos += 2), (token = SyntaxKind.PercentEqualsToken)
                        );
                    }
                    pos++;
                ...
                case CharacterCodes.asterisk:
                    if (text.charCodeAt(pos + 1) === CharacterCodes.equals) {
                        return (
                            (pos += 2), (token = SyntaxKind.AsteriskEqualsToken)
                        );
                    }
                    if (text.charCodeAt(pos + 1) === CharacterCodes.asterisk) {
                        if (
                ...
                case CharacterCodes.minus:
                    ...
                    if (text.charCodeAt(pos + 1) === CharacterCodes.equals) {
                        return (
                            (pos += 2), (token = SyntaxKind.MinusEqualsToken)
                        );
                    }
                    pos++;
                ...
                case CharacterCodes.slash:
                    ...
                    if (text.charCodeAt(pos + 1) === CharacterCodes.equals) {
                        return (
                            (pos += 2), (token = SyntaxKind.SlashEqualsToken)
                        );
                    }
                ...
                case CharacterCodes.lessThan:
                    ...
                    if (text.charCodeAt(pos + 1) === CharacterCodes.equals) {
                        return (
                            (pos += 2), (token = SyntaxKind.LessThanEqualsToken)
                        );
                    }
                    if (
                        ...
                        return (
                            (pos += 2), (token = SyntaxKind.LessThanSlashToken)
                        );
                    }
                    pos++;
                ...
                case CharacterCodes.equals:
                    ...
                    if (text.charCodeAt(pos + 1) === CharacterCodes.equals) {
                        ...
                        return (
                            (pos += 2), (token = SyntaxKind.EqualsEqualsToken)
                        );
                    }
                    if (
                        text.charCodeAt(pos + 1) === CharacterCodes.greaterThan
                    ) {
                        return (
                            (pos += 2),
                ...
                case CharacterCodes.question:
                    if (
                        ...
                        return (
                            (pos += 2), (token = SyntaxKind.QuestionDotToken)
                        );
                    }
                ...
                case CharacterCodes.caret:
                    if (text.charCodeAt(pos + 1) === CharacterCodes.equals) {
                        return (
                            (pos += 2), (token = SyntaxKind.CaretEqualsToken)
                        );
                    }
                    pos++;
                ...
                case CharacterCodes.backslash:
                    ...
                    if (
                        ...
                        tokenFlags |= TokenFlags.ExtendedUnicodeEscape;
                        tokenValue =
                            scanExtendedUnicodeEscape() + scanIdentifierParts();
                    ...
                    if (
                        ...
                        tokenFlags |= TokenFlags.UnicodeEscape;
                        tokenValue =
                            String.fromCharCode(cookedChar) +
                            scanIdentifierParts();
                        return (token = getIdentifierToken());
                    }
        ...
        for (
            ...
            if (!inBackticks) {
                if (ch === CharacterCodes.openBrace) {
                    ...
                    pos - 1 >= 0 &&
                    isWhiteSpaceSingleLine(text.charCodeAt(pos - 1)) &&
                    !(
                        pos + 1 < end &&
                        isWhiteSpaceLike(text.charCodeAt(pos + 1))
                    )
                ) {
...
const utf16EncodeAsStringWorker: (codePoint: number) => string = (String as any)
    .fromCodePoint
